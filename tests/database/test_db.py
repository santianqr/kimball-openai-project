...
def test_database_manager(dummy_data):
    ...
    assert list(query_result.columns) == [
        "id",
        "name",
        "age",
    ...
    assert (
        sql_file_result is not None
    ), "El resultado de la consulta del archivo SQL es None"
    assert (
        not sql_file_result.empty
    ), "El DataFrame de resultado del archivo SQL está vacío"
    assert list(sql_file_result.columns) == [
        "id",
        "name",
        "age",
    ], "Las columnas del DataFrame del archivo SQL no son las esperadas"
